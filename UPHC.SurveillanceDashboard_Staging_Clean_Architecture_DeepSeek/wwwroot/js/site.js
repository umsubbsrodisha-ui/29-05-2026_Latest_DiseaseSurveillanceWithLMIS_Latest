...
//6.
// Mini pie charts for facility cards
let miniChartInstances = {};  // ← Object to hold multiple charts

window.renderFacilityMiniPies = (facilityData) => {
    ...
    facilityData.forEach(f => {
        const canvas = document.getElementById(`pie-${f.facilityId}`);
        if (!canvas) return;

        const ctx = canvas.getContext('2d');

        // Store each chart with its facility ID as key
        miniChartInstances[f.facilityId] = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: f.labels,
                datasets: [{
                    data: f.data,
                    backgroundColor: colors.slice(0, f.data.length),
                    borderWidth: 1,
                    borderColor: "#fff"
                }]
            },
